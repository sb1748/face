...
class MainHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        if True not in matches:
            ...
            urllib.request.urlopen('http://39.108.65.xxx:8090/drty/sendPhoto?imgName='+name)
        else:
            num=matches.index(True) #如果已经存在人脸，就找出此人脸照片的索引
            imgname=know_people_list[num] #找出此人脸照片，获取照片名字
            #调用java接口，将照片名字作为参数
            urllib.request.urlopen('http://39.108.65.xxx:8090/drty/sendPhoto?imgName='+imgname)
